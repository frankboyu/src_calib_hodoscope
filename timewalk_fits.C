...
void WriteNA(ofstream &fout, ofstream &fout_ccdb, int counter) {
    ...
    fout << sep << na << sep << na << sep << na << sep << na << endl; 
    sep = "        ";
    fout_ccdb << counter << sep << 0.0 << sep << 0.0 << sep << 0.0 << sep << 0.0 << endl;
}
void WriteTimewalkFitResults(ofstream &fout, ofstream &fout_ccdb, int counter, double *y, double *dy, double min, double max, int run_numb) {
    ...
    stringstream ss2; ss2 << run_numb;
    if(counter<128 || (counter>178 && counter<257)){
      c.Print("results/results_timewalk/tw-fit-"+TString(ss2.str())+".pdf(", "pdf");
    }
    else if (counter==257){
      c.Print("results/results_timewalk/tw-fit-"+TString(ss2.str())+".pdf)", "pdf");
    }
    ...
    //fout << sep << f.GetParError(0) << sep << f.GetParError(1) << sep << f.GetParError(2) << sep << f.GetParError(3) << endl; 
    sep = "        ";
...
int timewalk_fits(int run_numb) {
    gStyle->SetOptFit(111);
    char file_name[300];
    sprintf(file_name, "timewalk-fits.txt");
    ofstream fout; fout.open(file_name);
    
    sprintf(file_name, "tw-corr-new.txt", run_numb);
    ofstream fout_ccdb; fout_ccdb.open(file_name);
    for (int i = 1; i <= 274; i++) { // counters
        stringstream ss; ss << i;
        const int N = 41; // pulse-height bins
        double y[N]; double dy[N];
        double max = -10.0; double min = 10.0;
        GetData("gaussian-fits-csv/counter_"+TString(ss.str())+".txt",y,dy,min,max);
        WriteTimewalkFitResults(fout,fout_ccdb,i,y,dy,min,max,run_numb);
    }
