...
def main(argv):
    ...
    #for RUN in range(MINRUN, MAXRUN + 1):
        ...
        if (str(RUN)[0:2]=='10'):
            INDATA_TOPDIR = '/mss/halld/RunPeriod-2022-05/rawdata'
        elif (str(RUN)[0:2]=='11'):
            INDATA_TOPDIR = '/mss/halld/RunPeriod-2022-08/rawdata'
        elif (str(RUN)[0:2]=='12'):
            INDATA_TOPDIR = '/mss/halld/RunPeriod-2023-01/rawdata'
        ...
        if (RUN in list_runs_evio[:,0]):
            FILENUM = '00' + str(int(list_runs_evio[np.where(list_runs_evio==RUN)[0]][0,1]))
        else:
            FILENUM = '000'
        
        for FILEPATH in file_list:
            if (FILEPATH[-8:-5]==FILENUM):
